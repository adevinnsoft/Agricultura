...
public partial class frmEmbarques : BasePage
{
    ...
    public static string obtineEmbarques()
    {
        ...
        {
            DataAccess dataaccess = new DataAccess();
            DataTable dt = dataaccess.executeStoreProcedureDataTable("sprAndroid_getEmbarqueHeader", new Dictionary<string, object>() { { "@idLider",  HttpContext.Current.Session["idUsuario"]} });
            ...
            else
            {
                ...
                foreach (DataRow item in dt.Rows)
                {
                    response += "<tr idEmbarque='" + item["idEmbarque"] + "' comentario='" + item["comentario"] + "' onClick='gvPartidas(this);'>"
                        + "<td>" + item["idEmbarque"] + "</td>"
                        //+ "<td>" + item["FechaOrigen"] + "</td>"//DateTime.Parse(item["FechaDestino"].ToString()).ToString("yyy-MM-dd")
                        + "<td>" + DateTime.Parse(item["FechaOrigen"].ToString()).ToString("yyy-MM-dd HH:mm") + "</td>"
                        + "<td>" + item["Origen"] + "</td>"
                        + "<td>" + item["Semana"] + "</td>"
                        + "<td>" + (item["TotalCajas"].ToString() == "" ? 0 : item["TotalCajas"]) + "</td>"
                        //+ "<td>" + item["Estado"] + "</td>"
                        + "</tr>";
                }
