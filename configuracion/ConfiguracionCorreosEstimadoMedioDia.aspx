<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/MasterPage.master" CodeFile="ConfiguracionCorreosEstimadoMedioDia.aspx.cs" Inherits="configuracion_ConfiguracionCorreosEstimadoMedioDia" %>
    ...
    <script type="text/javascript" src="../comun/scripts/jquery.blockUI.js"></script>
        ...
        function obtenerListaDeDistribucion() {
            PageMethods.obtenerListaDeDistribucion(function (response) {
                if (response[0] == '1') {
                    $('#gvEstimadoMedioDia').append(response[2].toString());
        ...
        function agregarCorreos(){
           var SAMaccount = $('.CuentaEstimadoMedioDia').val().trim();

           try {
               $.blockUI();
               PageMethods.agregarCorreos(SAMaccount,function(response){
                     if(response[0] == '1'){
                        $('#gvEstimadoMedioDia').append(response[2]);
                     }
                     else
                     {
                        popUpAlert(response[1],response[2]);
        ...
        function guardarListaDeDistribucion(){
         listaDeDistribucion = $('#table tbody tr td input[class="invisible"]').map(function () {
                    if ($(this).val() != '') {
                        return {
                            idCaptura : $('#gvEstimadoMedioDia tbody tr[idCaptura]').first().text() == '' ? <%=idCapturaDefault %> : $('#gvEstimadoMedioDia tbody tr[idCaptura]').first().text(),
                            etiqueta: $(this).val(),
                            correos: $('#gvEstimadoMedioDia tbody tr:not(.Cargado)').map(function () {//#gvPreHarvest tbody tr[class="Nuevo"]
                                if ($(this).text() != '') {
                                    return {
                                        correo: $(this).find('td[otherclass="Correo"]').text(),
                                        cuenta: $(this).find('td[otherclass="SAMaccount"]').text(),
                                        estado: $('#gvEstimadoMedioDia tbody tr[class="Nuevo"]').length > 0 ? 1 : 0
                                    }
                                }
                            }).get()  
                        }
        ...
        $(function () {
              ...
              $('#btnGuardarEstimadoMedioDia').click(function () { 
...
//            });
     ...
     <style type="text/css">
         ...
         .configCorreosEstimadoMedioDia h3
         {
             width: 100%;
             min-width: 100%;
             background: #F4D101;
         ...
         .configCorreosEstimadoMedioDia h3.open
         {
             width: 100%;
             background: #F4D101;
             padding: 10px;
         ...
         img#btnAgregarEstimadoMedioDia
         {
             position: relative;
             left: 210px;
             bottom: 40px;
    ...
    <div class="container">
        <asp:UpdatePanel ID="UpdatePanel1" runat="server">
             <ContentTemplate>
                   ...
                   <h1><asp:Label ID="Label1" runat="server">Configuracion de Correos para Estimado Medio Dia</asp:Label></h1>
                   <div class="configCorreosEstimadoMedioDia">
                      <h3 class="accHead"><asp:Label ID="lblEstimadoMedioDia" runat="server" Text="Estimado Medio Dia"></asp:Label></h3>
                      <div class="accBody">
                        <table id="table">
                            <tr>
                                <td class="correos">
                                    ...
                                    <asp:Label ID="lblCuentaEstimadoMedioDia" runat="server"  Text="Cuenta"></asp:Label>
                                    <asp:TextBox ID="txtCuentaEstimadoMedioDia" runat="server" class="CuentaEstimadoMedioDia" MaxLength="200"></asp:TextBox>
                                    ...
                                    <table id="gvEstimadoMedioDia" class="gridView">
                                        <thead>
                                        <tr>
                      ...
                      <input  id="btnGuardarEstimadoMedioDia" class="Guardar" type="button"  value="Guardar"/> 
