<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/MasterPage.master" CodeFile="ConfiguracionCorreosMonitoreo.aspx.cs" Inherits="configuracion_ConfiguracionCorreosMonitoreo" %>
    ...
    <script type="text/javascript" id="funcionamientoDePantalla">
        ...
        function obtenerListaDeDistribucion() {
            PageMethods.obtenerListaDeDistribucion(function (response) {
                if (response[0] == '1') {
                    $('#gvMonitoreo').append(response[2].toString());
        ...
        function agregarCorreos(){
           var SAMaccount = $('.CuentaMonitoreo').val().trim();

           try {
               $.blockUI();
               PageMethods.agregarCorreos(SAMaccount,function(response){
                     if(response[0] == '1'){
                        $('#gvMonitoreo').append(response[2]);
                     }
                     else
                     {
                        popUpAlert(response[1],response[2]);
       ...
       function guardarListaDeDistribucion(){
         listaDeDistribucion = $('#table tbody tr td input[class="invisible"]').map(function () {
                    if ($(this).val() != '') {
                        return {
                            idCaptura : $('#gvMonitoreo tbody tr[idCaptura]').first().text() == '' ? <%=idCapturaDefault %> : $('#gvMonitoreo tbody tr[idCaptura]').first().text(),
                            etiqueta: $(this).val(),
                            correos: $('#gvMonitoreo tbody tr:not(.Cargado)').map(function () {//#gvPreHarvest tbody tr[class="Nuevo"]
                                if ($(this).text() != '') {
                                    return {
                                        correo: $(this).find('td[otherclass="Correo"]').text(),
                                        cuenta: $(this).find('td[otherclass="SAMaccount"]').text(),
                                        estado: $('#gvMonitoreo tbody tr[class="Nuevo"]').length > 0 ? 1 : 0
                                    }
                                }
                            }).get()  
                        }
       ...
       <style type="text/css">
           ...
           .configCorreosMonitoreo h3.open
           {
               width: 100%;
               background: #F4D101;
               padding: 10px;
    ...
    <div class="container">
        <asp:UpdatePanel ID="UpdatePanel1" runat="server">
             <ContentTemplate>
                   ...
                   <h1><asp:Label ID="Label1" runat="server">Configuracion de Correos para Monitoreo</asp:Label></h1>
                   <div class="configCorreosMonitoreo">
                      <h3 class="accHead"><asp:Label ID="lblMonitoreo" runat="server" Text="Monitoreo"></asp:Label></h3>
                      <div class="accBody">
                        <table id="table">
                            <tr>
                                <td class="correos">
                                    ...
                                    <asp:Label ID="lblDestinatariosMonitoreo" runat="server"  Text="Cuenta"></asp:Label>
                                    <asp:TextBox ID="txtCuentaMonitoreo" runat="server" class="CuentaMonitoreo" MaxLength="200"></asp:TextBox>
                                    ...
                                    <%--<img src="../comun/img/add-icon.png" id="btnAgregarMonitoreo" class="botonAgregar">--%>
                                    <table id="gvMonitoreo" class="gridView">
                                        <thead>
                                        <tr>
                      ...
                      <input  id="btnGuardarMonitoreo" type="button"  value="Guardar"/> 
