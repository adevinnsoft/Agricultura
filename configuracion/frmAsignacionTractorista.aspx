<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    CodeFile="frmAsignacionTractorista.aspx.cs" Inherits="frmAsignacionTractorista" %>
...
<asp:content id="Content1" contentplaceholderid="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function gvInvernaderos() {
            try {
                $.blockUI();

                PageMethods.tablaInvernadero(function (result) {
                    $("#<%=divInvernaderos.ClientID %>").html(result);
                    $("#<%=divInvernaderos.ClientID %>").show();
                    if ($("#tablaInvernadero").find("tbody").find("tr").size() >= 1) {
                        var pagerOptions = { // Opciones para el  paginador
                            container: $("#pagerTractoristaInv"),
        ...
        function asignacionGuardar() {
            $("input:checkbox.tractoristaInv").change(function () {
                //$.blockUI();
                var idInvernadero = $(this).val().split('|')[0];
                var idTractoristas = $(this).val().split('|')[1];
                var activo = $(this).prop("checked");

                PageMethods.guardaRelacion(idInvernadero, idTractoristas, activo, function (result) {
                    try {
                        if (result[0] == "OK") {
                            $("#img_" + idInvernadero + "_" + idTractoristas).fadeIn().fadeOut();
                        }
                        //$.unblockUI();
                    } catch (err) {
                        console.log(err); $.unblockUI();
        ...
        function allSelect(idTractorista, selected) {
            $("input:checkbox.tractoristaInv[id$='|" + idTractorista + "']").each(function () {
                if (selected && $(this).prop("checked") == false) {
                    $(this).click();
                } else {
                    if (selected == false && $(this).prop("checked")) {
                        $(this).click();
                    }
                }
            });
...
<asp:content id="Content2" contentplaceholderid="ContentPlaceHolder1" runat="Server">
    <div class="container">
        ...
        <h1>
            <asp:label id="lblTitulo" runat="server" text="Asignación de Tractoristas"></asp:label>
        </h1>
        <table class="index">
            <tr>
                <td align="left" colspan="2">
                    <h2>
                        <asp:literal id="ltSubTitulo" text="Lista de Invernaderos y Tractoristas" runat="server">
                        </asp:literal>
                    </h2>
                </td>
            </tr>
            <tr>
                <td style="vertical-align: top;">
                    <div id="gvInvernaderos" class="grid">
                        <div id="pagerTractoristaInv" class="pager" style="width: 100%; min-width: 100%;
                            display: none;">
                        ...
                        <div id="divInvernaderos" runat="server" />
                    </div>
                </td>
            </tr>
        </table>
